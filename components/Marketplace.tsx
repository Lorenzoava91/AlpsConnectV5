...
import { MapPin, Calendar, ArrowRight, List, Map as MapIcon, X, MessageCircle, Send, Search, Info, ChevronDown, ChevronUp, Radar } from 'lucide-react';
...
// --- COMPONENT: Map Controller to track center ---
...
const Marketplace: React.FC<Props> = ({ trips, onRequestJoin, lang }) => {
  ...
  // Radius Search State
  const [searchRadius, setSearchRadius] = useState<number>(0); // 0 = Anywhere
  const [mapCenter, setMapCenter] = useState<{lat: number, lng: number}>({ lat: 45.8, lng: 9.0 }); // Default North Italy
  ...
  const filteredTrips = useMemo(() => {
    return trips.filter(trip => {
      ...
      if (searchRadius > 0) {
          const dist = calculateDistance(mapCenter.lat, mapCenter.lng, trip.coordinates.lat, trip.coordinates.lng);
          radiusMatch = dist <= searchRadius;
      }

      return categoryMatch && dateMatch && locationMatch && radiusMatch;
    });
  }, [trips, selectedCategory, dateRange, locationQuery, searchRadius, mapCenter]);
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-20">
      ...
      {/* FILTERS BAR */}
      <div className="sticky top-4 z-30 space-y-3 pointer-events-none">
        {/* Row 1: Categories - Horizontal Scrollable List for Mobile */}
        <div className="flex w-full overflow-x-auto gap-2 px-1 pb-1 no-scrollbar pointer-events-auto">
        ...
        <div className="flex flex-col md:flex-row gap-3 bg-white/80 backdrop-blur-md p-3 rounded-2xl shadow-sm border border-slate-200 pointer-events-auto">
           
           {/* Location Input & Radius */}
           <div className="flex-1 md:flex-[1.5] flex gap-2">
               {/* Search Text */}
               <div className="flex-1 bg-slate-50 rounded-xl p-1 border border-slate-200 flex items-center gap-2 group focus-within:ring-2 focus-within:ring-slate-200 transition-all">
               ...
               {/* Radius Selector */}
               <div className="bg-slate-50 rounded-xl p-1 border border-slate-200 flex items-center gap-2 px-2 shrink-0">
                   <Radar size={16} className="text-slate-400" />
                   <select 
                      value={searchRadius}
                      onChange={(e) => setSearchRadius(Number(e.target.value))}
                      className="bg-transparent text-sm font-bold text-slate-700 outline-none w-full md:w-auto"
                   >
                      <option value={0}>{t.anywhere}</option>
           ...
           <div className="flex bg-slate-100 p-1 rounded-xl shrink-0">
             <button 
               ...
               className={`flex items-center justify-center gap-2 px-3 md:px-4 py-2 rounded-lg text-sm font-medium transition-all flex-1 md:flex-none ${viewMode === 'list' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <List size={16} /> <span className="hidden md:inline">{t.list}</span>
             </button>
             <button 
               onClick={() => setViewMode('map')}
               className={`flex items-center justify-center gap-2 px-3 md:px-4 py-2 rounded-lg text-sm font-medium transition-all flex-1 md:flex-none ${viewMode === 'map' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <MapIcon size={16} /> <span className="hidden md:inline">{t.map}</span>
             </button>
           </div>
        </div>
      </div>
      ...
      {/* MAP VIEW */}
      {viewMode === 'map' && (
        <div className="bg-slate-100 rounded-3xl overflow-hidden h-[600px] relative border border-slate-200 shadow-inner">
            <MapContainer 
            ...
            >
                ...
                {/* Track Map Center Move */}
                <MapController onCenterChange={(lat, lng) => setMapCenter({ lat, lng })} />
                ...
                {filteredTrips.map(trip => (
                    <Marker 
                        ...
                        eventHandlers={{
                            click: () => {
                                // Explicitly handle click if needed, though Popup usually handles it.
                            }
                        }}
                    >
            ...
            {/* Radius Indicator Warning (if searching radius but looking elsewhere) */}
            {searchRadius > 0 && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[400] bg-slate-900/80 backdrop-blur-md text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg flex items-center gap-2 animate-in fade-in slide-in-from-top-4">
                    <Radar size={14} className="animate-pulse text-blue-400" />
                    {t.searchArea}: {searchRadius} km
                </div>
            )}
        </div>
      )}
        ...
        <>
          {filteredTrips.length === 0 ? (
            <div className="text-center py-20 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
               ...
               <button 
                  onClick={() => {setSelectedCategory(CATEGORY_ALL); setDateRange({start: '', end: ''}); setLocationQuery(''); setSearchRadius(0);}}
      ...
      {/* Contact Modal */}
      {contactModalTrip && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[1200] flex items-center justify-center p-4 animate-in fade-in duration-200">
